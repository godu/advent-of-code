...
import Data.List (elem, find, nub, unfoldr)
...
candidates puzzle =
  ( \guard ->
    ...
    <$> candidates
  where
    candidates = (guard puzzle :) $ guard <$> generation puzzle
...
process2 :: Puzzle -> Int
process2 = length . filter id . parMap rpar (hasLoop . generation) . candidates
