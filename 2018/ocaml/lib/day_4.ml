...
type state = WakeUp | FallAsleep | BeginShift of int
...
let extract_time_state s =
  let matches = s |> Re.exec record_regexp |> Re.get_all in
  (matches.(1), matches.(2))

let%test _ =
  "[1518-11-01 00:00] Guard #10 begins shift" |> extract_time_state
...
let record_of_string (recordString : string) : record =
  ...
  (timeString |> Time.of_string, stateString |> state_of_string)

let%test _ =
  "[1518-11-01 00:00] Guard #10 begins shift" |> record_of_string
  = (Time.of_string "1518-11-01 00:00", BeginShift 10)

let%test _ =
  "[1518-11-01 00:05] falls asleep" |> record_of_string
  = (Time.of_string "1518-11-01 00:05", FallAsleep)

let%test _ =
  "[1518-11-01 00:25] wakes up" |> record_of_string
  = (Time.of_string "1518-11-01 00:25", WakeUp)

let%test _ =
  "[1518-11-01 00:30] falls asleep" |> record_of_string
  = (Time.of_string "1518-11-01 00:30", FallAsleep)

let%test _ =
  "[1518-11-01 00:55] wakes up" |> record_of_string
  = (Time.of_string "1518-11-01 00:55", WakeUp)

let%test _ =
  "[1518-11-01 23:58] Guard #99 begins shift" |> record_of_string
  = (Time.of_string "1518-11-01 23:58", BeginShift 99)

let%test _ =
  "[1518-11-02 00:40] falls asleep" |> record_of_string
  = (Time.of_string "1518-11-02 00:40", FallAsleep)

let%test _ =
  "[1518-11-02 00:50] wakes up" |> record_of_string
  = (Time.of_string "1518-11-02 00:50", WakeUp)

let%test _ =
  "[1518-11-03 00:05] Guard #10 begins shift" |> record_of_string
  = (Time.of_string "1518-11-03 00:05", BeginShift 10)

let%test _ =
  "[1518-11-03 00:24] falls asleep" |> record_of_string
  = (Time.of_string "1518-11-03 00:24", FallAsleep)

let%test _ =
  "[1518-11-03 00:29] wakes up" |> record_of_string
  = (Time.of_string "1518-11-03 00:29", WakeUp)

let%test _ =
  "[1518-11-04 00:02] Guard #99 begins shift" |> record_of_string
  = (Time.of_string "1518-11-04 00:02", BeginShift 99)

let%test _ =
  "[1518-11-04 00:36] falls asleep" |> record_of_string
  = (Time.of_string "1518-11-04 00:36", FallAsleep)

let%test _ =
  "[1518-11-04 00:46] wakes up" |> record_of_string
  = (Time.of_string "1518-11-04 00:46", WakeUp)

let%test _ =
  "[1518-11-05 00:03] Guard #99 begins shift" |> record_of_string
  = (Time.of_string "1518-11-05 00:03", BeginShift 99)

let%test _ =
  "[1518-11-05 00:45] falls asleep" |> record_of_string
  = (Time.of_string "1518-11-05 00:45", FallAsleep)

let%test _ =
  "[1518-11-05 00:55] wakes up" |> record_of_string
...
let group_by_guard (records : (int * record list) list) :
  ...
  |> List.map ~f:(fun l ->
         ...
         let records = l |> List.map ~f:snd |> List.concat in
         (id, records) )
...
let group_by_sleep (records : record list) : record list list =
  records
  |> List.group ~break:(fun _ b ->
         match snd b with FallAsleep -> true | _ -> false )
...
let minutes_asleep_of_records (records : record list) : Time.t list =
  match records with
  | [(asleep_time, FallAsleep); (awake_time, WakeUp)] ->
      range_time asleep_time awake_time
  | _ -> []
...
let find_most_asleep_minute (minutes_asleep : int list) =
  ...
  |> List.group ~break:( <> )
  |> List.max_elt ~compare:(fun a b ->
         Int.compare (a |> List.length) (b |> List.length) )
  |> Base.Option.map ~f:(fun a -> (a |> List.hd_exn, a |> List.length))
  |> Base.Option.value ~default:(0, 0)

let%test _ =
  [5; 6; 7; 3; 4; 5; 4; 5; 6] |> find_most_asleep_minute |> ( = ) (5, 3)
