...
let print_claim claim =
  Printf.sprintf "Claim { id=%d; top=%d; left=%d; width=%d; height=%d; }" claim.id claim.top claim.left claim.width claim.height
  |> print_endline
let%test _ = "#123 @ 3,2: 5x4" |> claim_of_string = Some {id=123; left=3; top=2; width=5; height=4}
let%test _ = "#1 @ 1,3: 4x4" |> claim_of_string = Some {id=1; left=1; top=3; width=4; height=4}
let%test _ = "#2 @ 3,1: 4x4" |> claim_of_string = Some {id=2; left=3; top=1; width=4; height=4}
let%test _ = "#3 @ 5,5: 2x2" |> claim_of_string = Some {id=3; left=5; top=5; width=2; height=2}
...
let mark_square (row: int) (column: int) (id: int) (piece: piece): piece =
  let row = Array.get piece row in
  let cell = Array.get row column in
  Array.set row column (List.append cell [id]);
  piece
let%test _ = make_piece 3 2 |> mark_square 2 1 3 = [|[|[]; []|]; [|[]; []|]; [|[]; [3]|]|]
        ...
        (fun rowIndex row -> 
           ...
           | true ->
             ...
             |> Array.iteri 
               (fun columnIndex _ -> 
                  match 
                    columnIndex >= claim.left && columnIndex < (claim.left + claim.width)
                  with
                  | true ->
                    mark_square rowIndex columnIndex claim.id piece |> ignore
                  | false -> ()
               );
           | false -> ()
        );
...
let part_2 row_count column_count input = 
  ...
  let overlaped_ids = 
    claims
    |> mark_claim piece
    |> Array.fold_left (fun ids_set row -> 
        row
        |> Array.to_list
  ...
  Int_set.diff ids  overlaped_ids
  |> Int_set.find_first (fun _ -> true)
